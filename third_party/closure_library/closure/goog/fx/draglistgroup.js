...
goog.fx.DragListGroup.prototype.handlePotentialDragStart_ = function(e) {
  ...
  // Clean up these move listeners that suppress scrolling on drag end.
  goog.events.listen(goog.global.document.body, goog.events.EventType.MOUSEMOVE,
      this.handleMouseOrTouchMoveDuringDrag_, false, this);
  goog.events.listen(goog.global.document.body, goog.events.EventType.TOUCHMOVE,
      this.handleMouseOrTouchMoveDuringDrag_, false, this);
...
/**
 * Handles what happens when the mouse or touch moves during dragging. We
 * want to suppress scrolling when that happens.
 * @param {!goog.events.BrowserEvent} event The associated event.
 * @private
 */
goog.fx.DragListGroup.prototype.handleMouseOrTouchMoveDuringDrag_ = function(
    event) {
...
goog.fx.DragListGroup.prototype.handleDragStart_ = function(e) {
  // Prevent mobile scrolling when the user drags an effect chip.
...
goog.fx.DragListGroup.prototype.handleDragMove_ = function(dragEvent) {
  // Prevent the drag from scrolling in mobile.
  dragEvent.browserEvent.preventDefault();
...
goog.fx.DragListGroup.prototype.cleanup_ = function(opt_e) {
  // Re-enable scrolling on drag (the default behavior).
