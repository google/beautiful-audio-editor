 ...
 * Note(chizeng): This constructor was editted to take in an element.
 */
 ...
 * @param {!Element=} opt_element An optional element to use. It must have id
 *     {@code id}. If not provided, the field does a search in the DOM.
 * @constructor
 * @extends {goog.events.EventTarget}
 */
goog.editor.Field = function(id, opt_doc, opt_element) {
  ...
  /**
   * The deferred for the iframe being loaded.
   * @private {!goog.async.Deferred}
   */
  this.iframeLoadedDeferred_ = new goog.async.Deferred();
   ...
   * @protected
   */
  this.originalElement = opt_element ||
      this.originalDomHelper.getElement(this.id);
...
goog.editor.Field.prototype.addListener = function(type, listener, opt_capture,
                                                   opt_handler) {
  var self = this;
  this.iframeLoadedDeferred_.addCallback(function() {
    ...
    if (opt_handler) {
      self.eventRegister.listenWithScope(
          elem, type, listener, opt_capture, opt_handler);
    } else {
      self.eventRegister.listen(elem, type, listener, opt_capture);
    }
  });
};
...
goog.editor.Field.prototype.focus = function() {
  ...
  this.iframeLoadedDeferred_.addCallback(function() {
    if (!goog.editor.BrowserFeature.HAS_CONTENT_EDITABLE &&
      ...
      self.getEditableDomHelper().getWindow().focus();
    } else {
...
goog.editor.Field.prototype.iframeFieldLoadHandler = function(iframe,
  ...
  // Note(chizeng): This was changed to have the iframe load first.
  var self = this;
  iframe.onload = function() {
    ...
    var body = doc.body;
    self.setupFieldObject(body);
    if (!goog.editor.BrowserFeature.HAS_CONTENT_EDITABLE &&
        self.usesIframe()) {
      self.turnOnDesignModeGecko();
    }
    self.handleFieldLoad();
    self.iframeLoadedDeferred_.callback();
  };
